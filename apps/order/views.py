...
# 前端 place_order.html ajax 传递的参数:地址id(addr_id) 支付方式(pay_method) 用户要购买的商品id字符串(sku_ids)
# 悲观锁
# order/commit/
		...
		try:
			...
			for sku_id in sku_ids:
				for i in range(3):
					...
					# 返回受影响的行数. res=1, 修改成功; res=0, 修改失败
...
class CheckPayView(View):
	...
	def post(self, request):
		...
		# 校验参数
		if not order_id:
			return JsonResponse({'res': 1, 'errmsg': '无效的订单id', 'err': '防重复'})
